...
class ForgotPassController: UIViewController, UITextFieldDelegate, LoginView {
	...
	func initializeFromStoryboard() -> ForgotPassController {
		let controller = AppStoryboard.Auth.instance.instantiateViewController(withIdentifier: ForgotPassController.storyboardID)
        guard let myController = controller as? ForgotPassController else { fatalError() }
        return myController
	}
