...
class RegisterController: UIViewController, GIDSignInDelegate, UITextFieldDelegate, LoginView {
	...
	@IBAction func registerEmail(_ sender: Any) {
		...
		if(email.isEmpty){
			...
			self.phoneTF.showErrorWithText(errorText: "Enter your phone number")
		}else if(name.isEmpty){
			self.nameTF.showErrorWithText(errorText: "Enter your name")
		}else if(email.isValidEmail()){
			self.startLoader()
			self.presenter.didRegisterWithEmail(email, password: password, phoneNumber: phone, name: name)
		}else{
